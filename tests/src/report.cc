...
int main() try {
  ...
  constexpr std::size_t kNumSamples = 10000;
  ...
  for (float x = kMin; x < kMax; x += kStride) {
    ...
    std::uint32_t first_bit = (diff ? 31 - __builtin_clz(diff) : -1);
  ...
  for (float x = kMin; x < kMax; x += kElementsInVector * kStride) {
    ...
    __m512i pos =
        _mm512_sub_epi32(_mm512_set1_epi32(31), _mm512_lzcnt_epi32(diff));
    __m512i first_bit =
